...
from value_mix.agent_model import RNNModel, _get_size
from value_mix.value_mix_loss import ValueMixLoss
from value_mix.ma_controler import mac
    ...
    def __init__(self, obs_space, action_space, config):
        ...
        if config["mixer"] is None:
            ...
            self.target_mixer = QMixer(
                ...
                config["mixing_embed_dim"]).to(self.device)
        elif config["mixer"] == 'qmixv2':
            self.mixer = QMixerV2(self.n_agents, self.env_global_state_shape,
                                config["mixing_embed_dim"]).to(self.device)
            self.target_mixer = QMixerV2(
                self.n_agents, self.env_global_state_shape,
                config["mixing_embed_dim"]).to(self.device)
        elif config["mixer"] == "vdn":
            self.mixer = VDNMixer().to(self.device)
            self.target_mixer = VDNMixer().to(self.device)
        elif config["mixer"] == "qatten":
            print('using qatten')
            self.mixer = QattenMixer(self.n_agents, self.env_global_state_shape).to(self.device)
            self.target_mixer = QattenMixer(self.n_agents, self.env_global_state_shape).to(self.device)
        elif config["mixer"] == "qplex":
            print('using qplex')
            # n_agents, state_shape, mixing_embed_dim, n_actions):
            self.mixer = DMAQer(self.n_agents, self.env_global_state_shape,
                                config["mixing_embed_dim"],
                                self.n_actions).to(self.device)
            self.target_mixer = DMAQer(self.n_agents, self.env_global_state_shape,
                                config["mixing_embed_dim"],
                                self.n_actions).to(self.device)
        else:
        ...
        self.loss = ValueMixLoss(self.model, self.target_model, self.mixer,
        ...
        if config["optimiser"] == "RMSprop":
            ...
            raise ValueError('{} does not support.'.format(config["optimiser"]))
